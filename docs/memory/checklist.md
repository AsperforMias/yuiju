...
完成统一 MemoryQueryRouter，把 episode / fact / plan / auto 路由能力从当前工具函数收敛成独立模块。
给 memory-search 补齐完整输入输出契约：counterpartyName、topK、统一 evidenceIds、metadata。
在 world 决策链正式接入按需记忆查询，而不只是最近行为历史。
在 message 聊天链路正式接入按需记忆查询，形成和 world 对齐的使用方式。
P1
